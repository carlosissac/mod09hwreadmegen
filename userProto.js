...
UserProto.prototype.formatInput = function() {
    ...
    const snapshot = `## Snapshot\n![image](./assets/${this.staticInput.snapshot})\n`
    ...
    this.staticInput = await inquirer.prompt([
        {
            type : `input`,
            name : `title`,
            message : `PROJECT TITLE`.yellow,
            validate : this.validateInput
        },
        {
            type : `editor`,
            name : `description`,
            message : `PROJECT DESCRIPTION`.yellow,
            validate : this.validateEditor
        },
        {
            type : `input`,
            name : `snapshot`,
            message : `SNAPSHOT FILE NAME (should already exists in ./assests folder)`.yellow,
            validate : this.validateInput
        },
        {
            type: `list`,
            name: `license`,
            message: `LICENSE TYPE`.yellow,
            choices: [`Apache License 2.0`, `GNU GPLv3`, `MIT`, `ISC`, `GNU GPLv2`],
        },        
        {
            type: `editor`,
            name: `contributing`,
            message: `CONTRIBUTION TASKS`.yellow,
            validate : this.validateEditor
        }
    ])
    this.exec_lock++
    ...
    this.rec_propmts = [
        {
            ...
            message : `Write ${this.real_tarPropUppSing} item?`.yellow,
            validate: this.validateInput
        },
        {
            type : `confirm`,
            name : `askAgain`,
            message : `Write another ${this.real_tarPropUppSing} item?`.yellow, 
            default : true
        }
    ]
}
...
//iproto.getStaticInput();
//iproto.recursiveExec();
